...
<%
	...
	// trimDirectiveWhitespaces="true" 문서 공백이 있으면 error 가 나서 자동으로 처리 되게 해주는 문법
	...
	// XML 생성 (page 지시자 contentType 꼭 application/json 설정)
	...
	for(User1VO vo : users){
		...
		eUser.addContext(eUid);
		eUser.addContext(eName);
		eUser.addContext(eBirth);
		eUser.addContext(eHp);
		eUser.addContext(eAge);

		eUser.addContext(eUser);
		
	}
	// 같은 데이터지만 포맷이 다름 비교해보기
	// XML 출력 (page 지시자 contentType 꼭 text/xml 설정, trimDirectiveWhitespaces="true" 설정)
	doc.setRootElement(eUser);
	XMLOutputFactory outputter = new XMLOutputFactory(Format.getPretty);
